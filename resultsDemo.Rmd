...
### R packages
The `bcdata` and `bcmaps` packages now provide an easy way to query, download and visualize RESULTS data through the R interface. This short vignette will demonstrate a few ways that we can do this.  These packages have been developed by Andy Teucher, Sam Albers, Stephanie Hazlitt and others, and more information can be found here: https://bcgov.github.io/bcdata/
...
treesDPG %>% 
  ...
  mutate(age=2020-REFERENCE_YEAR+S_SPECIES_AGE_1+1) %>% 
...
Let's map blocks older than 20 years:
```{r}
